...
class SimpleTestWithNN {
	constructor () {
		this.name = "SIMPLETESTWITHNN";
		this.title = "Simple test with Neural Net";
		this.deckType = enumNames.NN_WORDS;
	...
	handle (answer, deck, tempDeck) {
		...
		if (!helper.checkLanguage(answer, "ru"))	{
			result.error = true;
			result.errorText = "The language should be russian";
			return result;
		}
		this.questionCount++;
		...
		if (answer == currentRow.translation) {
			result.questionResult = true;
			result.questionResultText = `Right (${currentRow.translation})`;
			tempDeck.splice(0, 1);
		} else {
			result.questionResult = false;
			result.questionResultText = `Wrong (${currentRow.translation})`;
			this.wrongCount++;
...
module.exports = new SimpleTestWithNN();
